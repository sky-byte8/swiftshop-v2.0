...
import com.swiftshop.cart.domain.vo.CartVO;
import com.swiftshop.cart.mapper.CartMapper;
...
/*
    ...
    public void addItem2Cart(CartFormDTO cartFormDTO) {
        ...
        cart.setNum(cartFormDTO.getNum() == null ? 1 : Math.max(1, cartFormDTO.getNum()));
        // 3.2.保存当前用户
    ...
    @Override
    public void removeByUserIdAndItemIds(Long userId,List<Long> itemIds) {
        // 1.构建删除条件，userId和itemId
        QueryWrapper<Cart> queryWrapper = new QueryWrapper<Cart>();
        queryWrapper.lambda()
    ...
    private void checkCartsFull(Long userId) {
        ...
        int maxItems = cartProperties.getMaxItems() == null ? 10 : cartProperties.getMaxItems();
        if (count >= maxItems) {
            throw new BizIllegalException(
                    StrUtil.format("用户购物车商品不能超过{}件", maxItems));
        }
    }
